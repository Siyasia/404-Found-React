...
export default function Signup() {
  ...
  const handleSignUp = async (event) => {
    ...
    if (!trimmedName || !role || !password || !trimmedEmail) {
      setError('Please fill in all required fields.');
      return;
    }

    if (role === 'user' && !trimmedUsername) {
      setError('Please choose a username for the user account.');
      return;
    }
    ...
    const response = await signupAdult('50', trimmedName, role, trimmedEmail, password, trimmedUsername);
    ...
    if (response.status_code !== 200 || !response.user) {
      const errorMsg =
        ...
        'Failed to sign up. Email may already be associated with another account.';
      setError(errorMsg);
      return;
    }
        ...
        <aside className="ns-auth-hero">
          ...
          <div className="ns-auth-highlight-grid">
            ...
            <div className="ns-auth-highlight">
              ...
              <span>Use a strong password so the account is ready for use.</span>
            </div>
          </div>
